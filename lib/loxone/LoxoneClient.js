...
class LoxoneClient extends EventEmitter {
	...
	// An EFM carries power flows but not the battery SoC. Feed it the SoC from a
	// configured control (efmSocControl) or, failing that, from a lone
	// EnergyManager2 — so the EFM's battery node can show the charge %.
	_linkEfmSoc() {
		const efms = [...this.displaySet]
			.map((u) => this.structure.getControl(u))
			.filter((c) => c && c.type === "EFM" && !(c.states && c.states.Ssoc));
		if (!efms.length) {
			return;
		}
		const socUuid = this._resolveSocStateUuid();
		if (!socUuid) {
			return;
		}
		efms.forEach((efm) => this.structure.linkState(efm.uuid, "Ssoc", socUuid));
	}

	_resolveSocStateUuid() {
		const pick = (sv) => (Array.isArray(sv) ? sv[0] : sv);
		if (this.opt.efmSocControl) {
			let c = null;
			try {
				c = this.structure.resolve(this.opt.efmSocControl);
			} catch (e) {
				return null;
			}
			const st = (c && c.states) || {};
			const uuid = pick(st.Ssoc) || pick(st.value) || pick(st[Object.keys(st)[0]]);
			return typeof uuid === "string" ? uuid : null;
		}
		const managers = this.structure.controlsByType("EnergyManager2").filter((c) => c.states && c.states.Ssoc);
		return managers.length === 1 ? pick(managers[0].states.Ssoc) : null;
	}
	...
	_resolveDisplay() {
		...
		(this.opt.controls || []).forEach((entry) => {
			try {
				const c = this.structure.resolve(entry);
				explicit.add(c.uuid);
				set.add(c.uuid);
			} catch (e) {
		...
		(this.opt.rooms || []).forEach((r) => this.structure.controlsInRoom(r).forEach((c) => set.add(c.uuid)));
		(this.opt.categories || []).forEach((c) => this.structure.controlsInCategory(c).forEach((ct) => set.add(ct.uuid)));
		if (this.opt.hideEfmChildren !== false) {
			this._absorbEfmChildren(set, explicit);
		}
		this.displaySet = set;
		this.display = [...set];
		if (warnings.length) {
	...
	// An Energy-Flow Monitor renders its own first-level balance, so the meters
	// it is built from would only clutter the grid as duplicate tiles. Drop them
	// from the display set — unless the user asked for one explicitly by name/UUID.
	_absorbEfmChildren(set, explicit) {
		const owned = new Set();
		set.forEach((uuid) => {
			const c = this.structure.getControl(uuid);
			if (c && c.type === "EFM") {
				this.structure.referencedControlUuids(uuid).forEach((u) => owned.add(u));
			}
		});
		owned.forEach((u) => {
			if (!explicit.has(u)) {
